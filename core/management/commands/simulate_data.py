...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument("--customers-per-year", type=int, default=500, help="Clientes novos por ano (média)")
        parser.add_argument("--products-per-year", type=int, default=50, help="Produtos novos por ano (média)")
    ...
    def get_seasonality_multiplier(self, current_date: date) -> float:
        ...
        # Black Friday
        if (current_date.month == BLACK_FRIDAY_MONTH and 
            BLACK_FRIDAY_START_DAY <= current_date.day <= BLACK_FRIDAY_END_DAY):
            m *= (np.random.uniform(BLACK_FRIDAY_VARIANCE_MIN, BLACK_FRIDAY_VARIANCE_MAX) * 
                  BLACK_FRIDAY_BASE_MULTIPLIER)
        
        # Cyber Monday
        elif (current_date.month == CYBER_MONDAY_MONTH and 
              CYBER_MONDAY_START_DAY <= current_date.day <= CYBER_MONDAY_END_DAY and 
              current_date.weekday() == 0):
            m *= np.random.uniform(CYBER_MONDAY_BOOST_MIN, CYBER_MONDAY_BOOST_MAX)
        
        # Natal
        elif (current_date.month == CHRISTMAS_MONTH and 
              CHRISTMAS_START_DAY <= current_date.day <= CHRISTMAS_END_DAY):
            m *= np.random.uniform(CHRISTMAS_BOOST_MIN, CHRISTMAS_BOOST_MAX)
        
        # Pós-Natal
        elif (current_date.month == CHRISTMAS_MONTH and 
              current_date.day >= POST_CHRISTMAS_START_DAY):
            m *= np.random.uniform(POST_CHRISTMAS_REDUCTION_MIN, POST_CHRISTMAS_REDUCTION_MAX)
        ...
        # Aniversário da empresa
            ...
            m *= np.random.uniform(COMPANY_ANNIVERSARY_BOOST_MIN, COMPANY_ANNIVERSARY_BOOST_MAX)
        
        return max(m, MIN_SEASONALITY_MULTIPLIER)

    def apply_cagr(self, base_volume: float, start_date: date, current_date: date) -> float:
        """
    ...
    def check_market_event(self, current_date: date) -> Optional[str]:
        ...
        if self.market_event and self.event_end_date and current_date < self.event_end_date:
        ...
        if random.random() < MARKET_EVENT_PROBABILITY:
            ...
            duration = random.randint(MARKET_EVENT_MIN_DURATION, MARKET_EVENT_MAX_DURATION)
    ...
    def apply_market_event(self, multiplier: float, market_event: Optional[str]) -> float:
        """
    ...
    def handle(self, *args, **options):
        ...
        for year_offset in range(years):
            ...
            self.stdout.write(f"  👥 Ano {year_date.year}: +{num_new_customers} clientes")
            ...
            for _ in range(num_new_customers):
                ...
                persona = np.random.choice(['OneTime', 'Leal', 'VIP'], p=[0.70, 0.20, 0.10])
                ...
                customer_metadata[len(all_customers)-1] = {
                    'persona': persona, 
                    'created_at': created_at,
                    'max_purchases': random.randint(30, 100) if persona == 'OneTime' else (random.randint(100, 300) if persona == 'Leal' else random.randint(500, 1500)),
                    'purchase_count': 0
                }

        # Bulk Create
        self.stdout.write(f"Salvando {len(all_products)} produtos e {len(all_customers)} clientes...")
        ...
        with tqdm(total=total_days) as pbar:
            while current_date <= today:
                ...
                if market_event:
                    self.stdout.write(f"  {current_date}: {market_event.upper()} evento de mercado")
                    ...
                    if valid_indices:
                        ...
                        for cust_idx in buyer_indices:
                            ...
                            for _ in range(n_items):
                                ...
                                probs_products = np.array(product_weights) / sum(product_weights)
                                prod_idx = np.random.choice(len(created_products), p=probs_products)
        ...
        for item in aggregated_by_day:
            ...
            pipe.set(f"faturamento:{dia_str}", float(item['total_faturamento']), ex=86400)
